...
use transformer::{run_async, Source};

#[bench]
fn bench_filter_async_pipe_1(b: &mut Bencher) {
  ...
  b.iter(|| {
    futures::executor::block_on(async {
      run_async(&source).collect::<Vec<DataValue>>().await;
    });
  });
}

#[bench]
fn bench_filter_async_pipe_20_sequentially(b: &mut Bencher) {
  let data = vec![
...
#[bench]
fn bench_map_async_pipe_1(b: &mut Bencher) {
  let data = vec![DataValue::from_pairs(vec![("a", DataItem::Number(1.0))])];
...
#[bench]
fn bench_map_async_pipe_20_sequentially(b: &mut Bencher) {
  let data = vec![
    DataValue::from_pairs(vec![("a", DataItem::Number(1.0))]),
    DataValue::from_pairs(vec![("a", DataItem::Number(2.0))]),
    DataValue::from_pairs(vec![("a", DataItem::Number(3.0))]),
...
#[bench]
fn bench_group_async_pipe_1(b: &mut Bencher) {
  let data = vec![DataValue::from_pairs(vec![("a", DataItem::Number(1.0))])];
...
#[bench]
fn bench_group_async_pipe_20_sequentially(b: &mut Bencher) {
  let data = vec![
    DataValue::from_pairs(vec![("a", DataItem::Number(1.0))]),
    DataValue::from_pairs(vec![("a", DataItem::Number(2.0))]),
    DataValue::from_pairs(vec![("a", DataItem::Number(3.0))]),
